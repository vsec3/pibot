...
class Guilds(commands.Cog):
    ...
    async def createguild(
        ...
        current_guild = await self.guilds_manager.get_user_guild(interaction.user.id)
        if current_guild:
        ...
        # Check cost
        if not await self.economy_manager.has_wallet(interaction.user.id, GUILD_CREATION_COST):
        ...
        guild_id = await self.guilds_manager.create_guild(
            interaction.user.id, display_name, image_url, privacy.value, password
        )
        ...
        # Deduct cost
        await self.economy_manager.deduct_wallet(interaction.user.id, GUILD_CREATION_COST)
        ...
        if achievements_cog:
            if await achievements_cog.achievements_manager.unlock_achievement(interaction.user.id, "guildmaster"):
                await achievements_cog._award_achievement(interaction.user.id, "guildmaster")
    ...
    async def browseguilds(self, interaction: discord.Interaction) -> None:
        guilds = await self.guilds_manager.list_guilds()
        if not guilds:
    ...
    async def joinguild(self, interaction: discord.Interaction, guild_id: str) -> None:
        ...
        guild = await self.guilds_manager.get_guild(guild_id)
        if not guild:
        ...
        if guild["privacy"] == "public":
            success, error = await self.guilds_manager.join_guild(interaction.user.id, guild_id)
            if success:
                await self.guilds_manager.save()
                ...
                achievements_cog = self.bot.get_cog("Achievements")
                if achievements_cog:
                    await achievements_cog.check_guild_achievements(interaction.user.id)
    ...
    async def guildview(self, interaction: discord.Interaction) -> None:
        guild_name = await self.guilds_manager.get_user_guild(interaction.user.id)
        if not guild_name:
            embed = embedbuildthing("Not in Guild", "You are not in a guild.", color=discord.Color.red())
            await interaction.response.send_message(embed=embed)
            return
        
        guild = await self.guilds_manager.get_guild(guild_name)
        if not guild:
    ...
    async def guildleaderboard(self, interaction: discord.Interaction) -> None:
        ...
        standings = await self.guilds_manager.get_guild_leaderboard(guild_name, self.economy_manager)
        if not standings:
    ...
    async def guildleave(self, interaction: discord.Interaction) -> None:
        left = await self.guilds_manager.leave_guild(interaction.user.id)
        if not left:
            embed = embedbuildthing("Not in Guild", "You are not in a guild.", color=discord.Color.red())
            await interaction.response.send_message(embed=embed)
            return
    ...
    async def guildkick(self, interaction: discord.Interaction, user: discord.Member) -> None:
        ...
        success, error = await self.guilds_manager.kick_member(interaction.user.id, user.id)
        if success:
            await self.guilds_manager.save()
    ...
    async def guilddisband(self, interaction: discord.Interaction) -> None:
        disbanded = await self.guilds_manager.disband_guild(interaction.user.id)
        if not disbanded:
    ...
    async def guildrename(self, interaction: discord.Interaction, new_name: str) -> None:
        renamed = await self.guilds_manager.rename_guild(interaction.user.id, new_name)
        if not renamed:
    ...
    async def guildcap(self, interaction: discord.Interaction, cap: int) -> None:
        ...
        set_cap = await self.guilds_manager.set_member_cap(interaction.user.id, cap_value)
        if not set_cap:
    ...
    async def guildtransfer(self, interaction: discord.Interaction, user: discord.Member) -> None:
        guild_id = await self.guilds_manager.get_user_guild(interaction.user.id)
        if not guild_id:
            embed = embedbuildthing("Not in Guild", "You are not in a guild.", color=discord.Color.red())
            await interaction.response.send_message(embed=embed)
            return
        ...
        success, error = await self.guilds_manager.transfer_ownership(interaction.user.id, user.id)
        if success:
            await self.guilds_manager.save()
